...
function Get-LatestReleaseStrictRun([string]$RepoSlug, [string]$Branch) {
  $args = @(
    "run",
    "list",
    "--workflow", "release-strict-final.yml",
    "--repo", $RepoSlug,
    "--limit", "20",
...
if ([string]::IsNullOrWhiteSpace($Token)) {
  try {
    ...
    if (-not [string]::IsNullOrWhiteSpace($ghToken)) {
      $Token = $ghToken.Trim()
      Write-Host "[release-strict-dispatch] Using token resolved from 'gh auth token'."
    }
  }
  catch {
...
$dispatchArgs = @(
  ...
  "--repo", $repoSlug,
  "--ref", $Ref,
  "-f", ("deploy_to_railway=" + $deployToRailwayValue),
  "-f", ("railway_environment=" + $RailwayEnvironment),
...
if ($null -eq $run -or [string]::IsNullOrWhiteSpace([string]$run.databaseId)) {
  if ($NoWaitForRun) {
    Write-Host "[release-strict-dispatch] Dispatched, but run id is not yet visible."
    exit 0
  }
...
Write-Host ("[release-strict-dispatch] Run id: " + $runId)
Write-Host ("[release-strict-dispatch] Run url: " + $runUrl)

if ($NoWaitForRun) {
  Write-Host "[release-strict-dispatch] NoWaitForRun enabled. Exiting after dispatch."
  exit 0
}
...
while ([datetime]::UtcNow -lt $deadline) {
  ...
  Write-Host ("[release-strict-dispatch] Run status: " + $status + " (conclusion=" + $conclusion + ")")

  if ($status -eq "completed") {
    if ($conclusion -eq "success") {
      Write-Host "[release-strict-dispatch] Workflow completed successfully."
      exit 0
    }
