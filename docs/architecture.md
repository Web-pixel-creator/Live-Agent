...
8.6. Case Workspace default Case Wiki focus is derived from repo-owned `highlights` plus backend-built `focusPack` until an operator explicitly selects a proof/question chip, keeping product-facing drilldown, handoff preview, and active chip state on the same compiled blocker/proof posture as the operator-safe snapshot.
