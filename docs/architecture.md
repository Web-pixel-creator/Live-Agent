...
3. `apps/api-backend`
   - REST control plane
   - Operator summary, repo-owned operator queue (`GET /v1/operator/queue`), and governance APIs
...
8. Repo-owned runtime surface contract in `api-backend`: `runtime surface inventory` mirrors agents, routes, playbooks, evidence lanes, and runtime/control-plane capabilities from source-backed registries, while `runtime surface readiness` overlays bootstrap doctor, runtime diagnostics, service/device coverage, evidence posture, and compact Case Wiki ingress provenance into one operator-safe `ready / degraded / critical` verdict. The same layer exposes a repo-owned runtime session replay mirror for selected-session status, approvals, replay counters, resume-ready or blocked-by posture, next operator action/target/workspace, primary-step state, step progress/path, latest verified proof pointer and stage, latest turn `contextSource/contextIngressSource`, latest verified proof `contextSource/contextIngressSource`, compact booking/follow-up/handoff metadata, workflow boundary summary, boundary owner, approval gate, recovery path hint, recovery handoff, recovery drill summary, nullable `live transport` evidence for the actually used `relay` vs `direct_live` path, latest first-audio/first-output latency, a compact fallback-event count, a root `evidenceSignature` envelope for tamper-evident exports, and workflow linkage through one operator-safe snapshot instead of stitching `/v1/sessions`, `/v1/events`, `/v1/runs`, and `/v1/runtime/workflow-config` by hand. When `contextSource=case_wiki`, that replay contract also preserves whether the active compiled snapshot came from `preserved_input_case_wiki` or `gateway_hydrated_case_wiki`, so operator surfaces can show ingress provenance without re-reading raw request envelopes. It now also exposes a repo-owned `Case Wiki` route (`GET /v1/runtime/case-wiki`) plus operator note append route (`POST /v1/runtime/case-wiki/notes`) so compiled case memory can be built from raw runtime evidence without inventing a second ad hoc storage plane.
...
8.3. In the frontend, `Operator Session Ops` owns replay loading/export, while the compact `Session Boundary` card shows the structured `After refresh` ladder and keeps resume posture, workflow boundary, proof, recovery, handoff, checklist, primary-step CTA, current `live transport` posture, and compact `Case Wiki` proof/turn ingress provenance visible in the first runtime scan.
8.4. `scripts/runtime-surface-snapshot.mjs` emits the inventory/readiness pair into `artifacts/runtime/runtime-surface-snapshot.json`; in live mode it also mirrors the same compact Case Wiki ingress provenance into `readiness.summary.workflow.caseWikiIngress`, while `scripts/runtime-surface-parity-check.mjs` validates the snapshot against `configs/runtime-surface-manifest.json`, and `scripts/runtime-surface-doc-drift-check.mjs` verifies that package/docs claims still match the repo-owned runtime-surface routes, scripts, and artifacts.
8.5. Repo-owned operator session-ops contract in the frontend + API audit lane: purpose declarations, session replay, compiled `Case Wiki` memory, and cross-agent discovery are exportable/inspectable and attach to high-risk operator audit records. The same compiled `Case Wiki` snapshot also feeds operator-facing `Case Wiki Evidence`, `Case Wiki Focused Handoff`, `Case Wiki Focused Routing`, and `Case Wiki Focused Remediation` preview cards inside `Operator Session Ops` and a compact `Case Workspace` summary card, so product-facing triage can reuse operator-safe compiled memory instead of re-reading raw replay lines; both surfaces can expose summary posture plus a compact evidence rail (`top proof`, `key entity`) from the repo-owned `highlights` block, a short repo-owned `evidencePack` (`proofs`, `entities`, `questions`, `sourceRefs`), a backend-built `handoffPack` for compact proof/question handoff posture, a backend-built `detailPack` for proof/question detail rows and per-item badges, a backend-built `routingPack` for compact proof/question route + CTA posture, a backend-built `actionPack` for copy-ready proof/question mini-actions plus per-focus `remediationDraft` payloads, a backend-built `focusPack` for focus summaries, chip labels, chip titles, drilldowns, and handoff previews, a backend-built `previewPack` for pack/ref/proof/question/handoff summary strings, a backend-built `workspacePack` for the top `Case Workspace` summary card fields plus compact open-question, timeline, and per-case cost strings, plus structured `workspacePack.costSummary` unit-economics evidence (`tokens`, `liveMinutes`, `uiExecutorMinutes`, `storageMb`, priced USD components when configured), a backend-built `operatorPreviewPack` for the compact `Case Wiki Overview` / `Case Wiki Evidence` / `Case Wiki Open Questions` / `Case Wiki Focused Remediation` / `Case Wiki Compliance` / `Case Wiki Audit` / `Case Wiki Timeline` operator panes, clickable proof/question focus chips sourced from `focusPack`, expandable proof/question detail rows, compact per-item badges, source-aware `Copy handoff` / `Copy refs` mini-actions, one-tap jumps into the operator focused routing block, source-linked handoff/routing previews plus a ready-to-run one-click routing CTA on the same compiled snapshot, a focused remediation draft when the operator narrows to one proof/question, and one top-level `compliance` posture for governance template, redaction, retention, and signing expectations, while exports can carry the currently focused handoff block, focused routing block, focused remediation draft, workspace card model, top-level compliance posture, audit trail, and operator preview payload when the operator narrows the case to one proof or question.
8.5.a. The same compiled Case Wiki layer also feeds a repo-owned operator queue route (`GET /v1/operator/queue`) that prioritizes blocking approvals, remediation drafts, runtime follow-ups, case handoff actions, and explicit compliance-enforcement blockers server-side, so frontend `Active Queue` ordering stays aligned with backend case memory instead of reimplementing queue heuristics only in the browser.
8.5.1. The same compiled snapshot now also carries a repo-owned `compliance` posture plus `auditLog`, alongside `operatorPreviewPack.compliance` and `operatorPreviewPack.audit`, so governance template/redaction/retention/signing posture, approval audit rows, operator note inserts, workflow refreshes, and runtime verification updates become one compact `Compliance` + `Audit` lane (`actor`, `source`, `action`, `field`, `oldValue`, `newValue`, `reason`, `sourceRefs`) instead of forcing downstream tooling to diff raw replay lines. `compliance.enforcement` adds the explicit operator/export verdict for signed-vs-unsigned posture and raw-ref redaction drift (`status`, `snapshotMode`, `rawRefCount`, `redactionSatisfied`, `signatureSatisfied`, `exportReady`, `blockingReasons`), and that same verdict is projected into the repo-owned operator queue preview so compliance blockers are first-class queue items.
...
8.7. `live-agent` conversation turns now accept compiled Case Wiki snapshots through `payload.input.caseWiki`, `caseWikiSnapshot`, `runtimeCaseWiki`, `compiledCaseWiki`, or `context.caseWiki` and place that compiled memory above session summaries and recent transcript turns in the runtime prompt. Frontend dispatch and realtime-gateway ingress treat the same alias set as one compiled-memory contract, so matching current-session snapshots are preserved instead of refetched while stale snapshots are replaced by a repo-owned current-session `caseWiki` payload before route handoff. Diagnostics expose `contextSource: "caseWiki"` plus the active focus, blocker, next action, summary, and source refs when this layer is present; raw transcript/session memory remains the fallback evidence lane. Orchestrator delegation preserves the same `caseWiki` payload so specialist agents do not lose compiled memory on route handoff.
...
8.9. Runtime diagnostics now produce an additive latency SLO rollup for `liveFirstAudioP95`, `navigatorStepP95`, and `caseWikiQueryP95`, using recent replay events when available and service operation p95 metrics as fallback evidence. The same diagnostics contract also exposes runtime evidence signing posture (`enabled`, `keyState`, `expectedSignatureStatus`, `publicKeyFingerprint`) plus the latest repo-owned `Case Wiki` routing ingress provenance (`preserved_input_case_wiki` vs `gateway_hydrated_case_wiki`) derived from recent runtime events, and raises `evidence_signing_key_unavailable` when the signer is enabled without a valid key. Breaches are projected into Runtime Guardrails `activeSignals` and the demo/release evidence lane so production proof covers live transport, UI action, compiled memory query latency, ingress provenance, and evidence-signing readiness instead of only service presence.
