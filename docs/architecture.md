...
8. Repo-owned runtime surface contract in `api-backend`: `runtime surface inventory` mirrors agents, routes, playbooks, evidence lanes, and runtime/control-plane capabilities from source-backed registries, while `runtime surface readiness` overlays bootstrap doctor, runtime diagnostics, service/device coverage, and evidence posture into one operator-safe `ready / degraded / critical` verdict; the same layer now also exposes a repo-owned runtime session replay mirror for selected-session status, approvals, replay counters, resume-ready or blocked-by posture, a human-readable next operator action, the latest verified proof pointer and stage, compact booking/follow-up/handoff metadata, a workflow boundary summary, boundary owner, approval gate, recovery path hint, recovery handoff, a repo-owned recovery drill summary for failed workflow boundaries, and workflow linkage through one operator-safe snapshot instead of stitching `/v1/sessions`, `/v1/events`, `/v1/runs`, and `/v1/runtime/workflow-config` by hand. In the frontend, Operator Console mirrors that replay snapshot twice: deeper `Operator Session Ops` owns loading/export, while the compact `Session Boundary` card keeps resume posture, workflow boundary, explicit `Boundary Owner`, `Approval Gate`, `Latest Proof`, `Recovery`, and `Handoff` rows visible in the first runtime scan, with its primary CTA following the current handoff target. `scripts/runtime-surface-snapshot.mjs` emits the inventory/readiness pair into `artifacts/runtime/runtime-surface-snapshot.json` for portable offline/local/CI inspection, `scripts/runtime-surface-parity-check.mjs` validates that artifact against `configs/runtime-surface-manifest.json`, and `scripts/runtime-surface-doc-drift-check.mjs` verifies that package/docs claims still match the repo-owned runtime-surface routes, scripts, and artifacts
