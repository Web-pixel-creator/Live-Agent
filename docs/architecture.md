...
8.5.a. The same compiled Case Wiki layer also feeds a repo-owned operator queue route (`GET /v1/operator/queue`) that prioritizes blocking approvals, remediation drafts, runtime follow-ups, and case handoff actions server-side, so frontend `Active Queue` ordering stays aligned with backend case memory instead of reimplementing queue heuristics only in the browser.
