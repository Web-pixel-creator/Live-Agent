...
8. Repo-owned runtime surface contract in `api-backend`: `runtime surface inventory` mirrors agents, routes, playbooks, evidence lanes, and runtime/control-plane capabilities from source-backed registries, while `runtime surface readiness` overlays bootstrap doctor, runtime diagnostics, service/device coverage, and evidence posture into one operator-safe `ready / degraded / critical` verdict; `scripts/runtime-surface-snapshot.mjs` emits that pair into `artifacts/runtime/runtime-surface-snapshot.json` for portable offline/local/CI inspection, and `scripts/runtime-surface-parity-check.mjs` validates that artifact against `configs/runtime-surface-manifest.json`
