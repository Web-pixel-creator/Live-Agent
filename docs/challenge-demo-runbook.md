...
25. Unified release evidence report: `artifacts/release-evidence/report.json` + `artifacts/release-evidence/report.md` (single-source summary for strict/artifact/deploy evidence lanes, including `pluginMarketplaceStatus`, `agentUsageStatus`, `runtimeGuardrailsSignalPathsStatus`, `liveTransportStatus`, `hostedDirectLiveProofStatus`, `caseWikiEvidenceSignatureStatus`, `caseWikiRoutingContextStatus`, `caseWikiGatewayHydrationStatus`, `caseWikiContextAdoptionStatus`, compact runtime-guardrails snapshot fields `summaryStatus` / `totalPaths` / `primaryPath`, compact live-transport runtime/session snapshot fields, compact hosted direct-live proof snapshot fields `generatedAt` / `generatedAtIsIso` / `freshnessStatus` / `freshnessSummary` / `freshnessAgeMinutes` / `freshnessMaxAgeHours` / `apiPublicUrlSource` / `replayEvidenceSource` / `firstAudioMs` / `firstOutputMs` / `fallbackEventCount` / `caseWikiSignatureStatus` / `latencyObserved`, compact `Case Wiki` signer counts `totalArtifacts` / `signedArtifacts` / `unsignedArtifacts`, compact `Case Wiki` routing-context proof fields `contextSource` / `ingressSource` / `focusId` / `route` / `mode`, dedicated `Case Wiki` gateway-hydration proof fields (`sessionId`, `noteEventId`, `questionId`, `questionMatched`, `noteSourceRefSeen`, `questionSuggestedNextStep`, `ingressSource`), aggregate `Case Wiki` adoption counters (`observedCount`, `caseWikiObservedCount`, `inputOnlyObservedCount`, `unknownObservedCount`, `caseWikiRate`), derived `deviceNodeUpdatesStatus`, and `providerUsage` for adapter provenance such as storyteller `tts`, optional `image_edit` continuity passes, and live-agent `research` citation-bearing provenance, plus compact provider snapshot fields `validated` / `activeSecondaryProviders` / `entriesCount` / `primaryEntry`, optional hosted deploy artifact inventory entries for `artifacts/deploy/direct-live-proof.{json,md,png}`, and optional GCP runtime proof snapshot fields after Cloud Run deploy. In local/dev unsigned mode the `Case Wiki` signature block stays `validated=true` but is surfaced as `caseWikiEvidenceSignatureStatus=warn`; only signed keyed evidence should surface as `pass`, and stale hosted proof older than the default 24-hour freshness window is downgraded to `hostedDirectLiveProofStatus=fail`.)
26. Unified release evidence manifest: `artifacts/release-evidence/manifest.json` + `artifacts/release-evidence/manifest.md` (artifact inventory for demo/perf/provenance/report outputs with required/present state and copied critical evidence statuses, including the `Case Wiki Routing Context`, `Case Wiki Gateway Hydration`, and aggregate `Case Wiki Context Adoption` proof blocks).
