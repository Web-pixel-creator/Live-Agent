...
That manifest now also carries optional compact deploy/publish provenance snapshot fields (`railwayDeploySummary*`, `repoPublishSummary*`, `repoPublishSummaryCaseWikiRuntimeSurfaceIngress*`) whenever the downloaded artifact bundle contains `artifacts/deploy/railway-deploy-summary.json` and/or `artifacts/deploy/repo-publish-summary.json`. The `repoPublishSummaryCaseWikiRuntimeSurfaceIngress*` set is sourced from the top-level `caseWikiRuntimeSurfaceIngress` object in `repo-publish-summary.json`, so artifact-revalidation consumers can read publish-owned runtime ingress provenance directly from the publish artifact.
