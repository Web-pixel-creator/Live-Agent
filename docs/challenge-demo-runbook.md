...
When the hosted frontend points to `https://live-agent-api-production.up.railway.app`, keep that API on the same release track with `npm run deploy:railway:api -- -Service "Live-Agent-API" -Environment production -ApiPublicUrl https://live-agent-api-production.up.railway.app` or the manual workflow `.github/workflows/railway-deploy-api.yml`. That lane verifies both `/healthz` and `/v1/runtime/live/capabilities`, and it deploys through `infra/railway/manifests/api-backend.railway.json` so the Railway build sees the full npm workspace instead of an isolated subdirectory.
