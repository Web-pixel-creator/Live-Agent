...
For the explicit browser lane, run `npm run demo:e2e -- -IncludeFrontend`. That variant starts `demo-frontend`, opens the real page in Playwright, forces `?livePreferredMode=direct_live`, opens the visible `Voice` tray, populates the session fields, clicks `Connect`, and writes `artifacts/demo-e2e/direct-live-browser-smoke.json` plus `direct-live-browser-smoke.png`. On hosted layouts where the long `Voice` tray can intercept pointer clicks, the helper now falls back to a repo-owned DOM click so the smoke still validates the backend replay path instead of failing on tray chrome. If direct live is unsupported the smoke reports `skipped`; if runtime says direct live is supported, the smoke requires backend replay proof from repo-owned `session_events`.
