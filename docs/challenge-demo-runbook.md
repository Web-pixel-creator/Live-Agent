...
Unified evidence output: helper/workflow also emit `artifacts/release-evidence/report.json`, `artifacts/release-evidence/report.md`, `artifacts/release-evidence/manifest.json`, and `artifacts/release-evidence/manifest.md` derived from `badge-details.evidence.*` statuses (including `runtimeGuardrailsSignalPaths`, `caseWikiEvidenceSignature`, and `caseWikiRoutingContext`) plus artifact inventory; `report.json` now also carries compact runtime-guardrails snapshot fields `summaryStatus`, `totalPaths`, and `primaryPath`, `liveTransport` snapshot fields for runtime-vs-session transport proof, compact `Case Wiki` signer fields, compact `Case Wiki` routing-context proof fields (`contextSource`, `focusId`, `route`, `mode`), plus provider snapshot fields `validated`, `activeSecondaryProviders`, `entriesCount`, and `primaryEntry`. Strict/artifact workflow job summaries also print the compact `Case Wiki` routing tuple (`contextSource`, `focusId`, `blocker`, `nextAction`, `route`, `mode`, `requestedIntent`, `routedIntent`) so routing adoption can be checked without opening the JSON artifact.
...
25. Unified release evidence report: `artifacts/release-evidence/report.json` + `artifacts/release-evidence/report.md` (single-source summary for strict/artifact/deploy evidence lanes, including `pluginMarketplaceStatus`, `agentUsageStatus`, `runtimeGuardrailsSignalPathsStatus`, `liveTransportStatus`, `hostedDirectLiveProofStatus`, `caseWikiEvidenceSignatureStatus`, `caseWikiRoutingContextStatus`, compact runtime-guardrails snapshot fields `summaryStatus` / `totalPaths` / `primaryPath`, compact live-transport runtime/session snapshot fields, compact hosted direct-live proof snapshot fields `apiPublicUrlSource` / `replayEvidenceSource` / `firstAudioMs` / `firstOutputMs` / `fallbackEventCount` / `caseWikiSignatureStatus` / `latencyObserved`, compact `Case Wiki` signer counts `totalArtifacts` / `signedArtifacts` / `unsignedArtifacts`, compact `Case Wiki` routing-context proof fields `contextSource` / `focusId` / `route` / `mode`, derived `deviceNodeUpdatesStatus`, and `providerUsage` for adapter provenance such as storyteller `tts`, optional `image_edit` continuity passes, and live-agent `research` citation-bearing provenance, plus compact provider snapshot fields `validated` / `activeSecondaryProviders` / `entriesCount` / `primaryEntry`, optional hosted deploy artifact inventory entries for `artifacts/deploy/direct-live-proof.{json,md,png}`, and optional GCP runtime proof snapshot fields after Cloud Run deploy).
26. Unified release evidence manifest: `artifacts/release-evidence/manifest.json` + `artifacts/release-evidence/manifest.md` (artifact inventory for demo/perf/provenance/report outputs with required/present state and copied critical evidence statuses, including the `Case Wiki Routing Context` proof block).
