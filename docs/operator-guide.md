...
5. Frontend remembers last active tab (`mla.demoFrontend.activeTab`) and restores it on reload; tab rail stays sticky during scroll and panel switches use short fade/slide transitions (disabled under `prefers-reduced-motion`) to preserve visual orientation; Operator Console also remembers last board mode (`mla.demoFrontend.operatorBoardMode`) for consistent demo/operator context
