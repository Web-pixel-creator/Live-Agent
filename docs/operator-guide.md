...
4. `Device Nodes`: device-node create/update/heartbeat controls, compact fleet summary (`total/online/degraded/offline/stale`), `List Filter` (`all/online/degraded/offline/stale`) with live `Showing X of Y` counters, and selectable node cards with status pills; empty/filtered-empty states provide direct recovery actions (`Use Demo Template`, `Show All Nodes`)
...
18. Export session evidence from frontend via the `Export Session` dropdown (`Markdown` / `JSON` / `Audio (WAV)`)
...
22. Live Negotiator status strip (`Status/Assistant/Run ID/User ID/Session State/Mode/PTT/Export`) uses high-contrast text, pill-state color mapping, concise export labels (`exported markdown/json/audio`), and compact inline wrap chips (with stacked fallback on narrow screens) for quick judge/operator scan
