...
22. Live Negotiator status strip (`Status/Assistant/Run ID/User ID/Session State/Mode/PTT/Export`) uses high-contrast text, pill-state color mapping, concise export labels (`exported markdown/json/audio`), and a single-row inline bar on desktop with adaptive wrap/stack fallback on narrower screens for quick judge/operator scan
