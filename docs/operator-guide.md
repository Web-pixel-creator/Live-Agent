...
Live session note: `Connection & Export` now keeps `Export current session` inside its own session-export surface, so Markdown/JSON/WAV downloads read as evidence for the active live session instead of a floating action. `Advanced Session Settings` and `Live Setup` also now use wider grouped fields and calmer status rows so runtime fields stay readable during demos. Before the relay websocket opens, frontend also runs a repo-owned `POST /v1/runtime/live/session-token` bootstrap handshake and folds that posture into the `Mode` pill, so the first fold already shows `voice • relay`, `voice • relay • direct_ready`, `voice • direct_live • direct_ready`, or `voice • relay • fallback` before a browser-direct live path is enabled. Once constrained browser-direct live is active, frontend persists compact proof through `POST /v1/runtime/live/session-events`, so operator replay can show backend-observed `direct_live` instead of only local transport state.
