...
test("focused evidence uses workspace-specific dormant lane and source context chips", () => {
  ...
  assert.ok(
    readmeSource.includes("dormant `Focused Evidence` context strip now also swaps to workspace-aware `Lane / Source / Next` copy"),
    "README should document workspace-specific dormant context strip copy",
  );
  assert.ok(
    operatorGuideSource.includes("dormant `Focused Evidence` context strip now also swaps to workspace-aware `Lane / Source / Next` copy"),
