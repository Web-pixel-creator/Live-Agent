...
test("runtime session replay mirror aggregates selected session replay, approvals, and workflow linkage", () => {
  ...
  assert.deepEqual(
    {
      ...
      targetSurface: primaryStep.targetSurface,
      targetLabel: primaryStep.targetLabel,
      workspace: primaryStep.workspace,
      ctaLabel: primaryStep.ctaLabel,
      phase: primaryStep.phase,
      runState: primaryStep.runState,
      actionMode: primaryStep.actionMode,
      surfaceState: primaryStep.surfaceState,
      needsRefresh: primaryStep.needsRefresh,
    },
    {
  ...
  assert.equal(primaryStep.refreshState, null);
  assert.equal(primaryStep.refreshAction, null);
  assert.equal(primaryStep.refreshTargetState, null);
...
test("runtime session replay mirror blocks resume when approval or active workflow boundary still owns the session", () => {
  ...
  assert.equal(primaryStep.refreshDisposition, "reopen_then_refresh");
  assert.equal(primaryStep.refreshEvidenceHint, "Recheck the latest approval gate evidence.");
  assert.equal(primaryStep.refreshOutcomeLabel, "Approval gate is current again.");
  assert.equal(primaryStep.refreshConfidence, "medium");
  assert.equal(primaryStep.refreshEscalationReadiness, "ready");
  assert.equal(primaryStep.refreshEscalationTarget?.targetSurface, "operator_workflow_control");
  ...
  assert.deepEqual(
    ...
    ["refresh", "refresh_escalation"],
  );
  assert.equal(refreshState.followupTree?.level, "refresh");
  assert.equal(refreshState.followupTree?.next?.level, "refresh_escalation");
  assert.equal(refreshState.followupTree?.next?.next, null);
  assert.equal(primaryStep.refreshAction?.action, "refresh_session_replay");
  assert.equal(primaryStep.refreshTargetState?.refreshScope, "gate");
