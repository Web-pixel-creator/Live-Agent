...
test("operator console exposes session ops purpose, replay, and discovery surfaces", () => {
  ...
  assert.match(readmeSource, /resume-ready|latest verified proof pointer|handoff|workflow boundary|recovery path|approval gate|boundary owner|recovery drill|next action target|next operator workspace/i);
  ...
  assert.match(operatorGuideSource, /resume-ready|latest verified proof pointer|handoff|workflow boundary|recovery path|approval gate|boundary owner|recovery drill|next action target|next operator workspace/i);
});
