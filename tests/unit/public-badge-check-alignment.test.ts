...
test("public badge check helper stays aligned across package, script, and docs", () => {
  ...
  assert.match(scriptRaw, /caseWikiGatewayHydration/);
  assert.match(scriptRaw, /caseWikiContextAdoption/);
  ...
  assert.match(scriptRaw, /caseWikiGatewayHydrationEvidenceRequired/);
  assert.match(scriptRaw, /caseWikiContextAdoptionEvidenceRequired/);
  ...
  assert.match(scriptRaw, /Case-wiki-gateway-hydration status \(badge evidence\):/);
  assert.match(scriptRaw, /Case-wiki-context-adoption status \(badge evidence\):/);
  assert.match(scriptRaw, /Case-wiki-context-adoption rate \(badge evidence\):/);
  ...
  assert.match(scriptRaw, /caseWikiGatewayHydration must prove websocket gateway hydration from backend case wiki note state with note\/question linkage and routing metadata/);
  assert.match(scriptRaw, /caseWikiContextAdoption must prove observed\+validated adoption with count conservation and caseWikiRate>=0\.95/);
  ...
  assert.match(scriptRaw, /pluginMarketplace must be validated with observed status, totals\/lifecycle\/conflicts\/signing consistency, permission bounds, and latest plugin\/signing\/ISO fields/);
