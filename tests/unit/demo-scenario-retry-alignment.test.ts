...
test("demo-e2e applies transient retry to flaky operator/ui/runtime scenarios", () => {
  ...
  assert.match(
    source,
    /Invoke-Scenario[\s\S]*-Name\s+"skills\.registry\.lifecycle"[\s\S]*-MaxAttempts\s+\$ScenarioRetryMaxAttempts[\s\S]*-RetryTransientFailures/,
  );
  assert.match(
    source,
  ...
  assert.match(
    source,
    /Invoke-Scenario[\s\S]*-Name\s+"gateway\.websocket\.item_truncate"[\s\S]*-MaxAttempts\s+\$ScenarioRetryMaxAttempts[\s\S]*-RetryTransientFailures/,
  );
  assert.match(
    source,
    /Invoke-Scenario[\s\S]*-Name\s+"gateway\.websocket\.item_delete"[\s\S]*-MaxAttempts\s+\$ScenarioRetryMaxAttempts[\s\S]*-RetryTransientFailures/,
  );
  assert.match(
    source,
  ...
  assert.match(source, /gatewayItemDeleteScenarioAttempts/);
