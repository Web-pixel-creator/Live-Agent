...
test("operator console exposes compact session boundary widget", () => {
  ...
  assert.match(readmeSource, /approval gate|boundary owner|recovery path|recovery drill|next action target|next operator workspace|checklist|remaining steps|primary step|step progress|step path|active|queued|runnable|blocked|openable|executable|primed|not_primed|needsrefresh|fresh|needs_refresh|refresh disposition|refresh evidence hint|refresh outcome|refresh confidence|refresh detour|refresh escalation|refresh escalation target|refresh escalation mode|refresh escalation cta|refresh escalation readiness|refresh escalation prep|refresh escalation open guard|refresh escalation fallback|refresh escalation fallback cta|refresh escalation fallback readiness|refresh escalation fallback prep|refresh escalation fallback open guard|refresh escalation fallback outcome|refresh escalation fallback confidence|refresh escalation fallback detour|refresh escalation fallback escalation|refresh escalation fallback escalation target|refresh escalation fallback escalation mode|refresh escalation fallback escalation cta|refresh escalation fallback escalation readiness|refresh escalation fallback escalation prep|refresh escalation fallback escalation open guard|refresh escalation fallback escalation fallback target|refresh escalation fallback escalation fallback cta|refresh escalation fallback escalation fallback readiness|refresh escalation fallback escalation fallback prep|refresh escalation fallback escalation fallback open guard|refresh escalation fallback escalation fallback outcome|refresh escalation fallback escalation fallback confidence|refresh escalation fallback escalation fallback detour|refresh escalation fallback escalation fallback escalation|approval escalation|recovery escalation|workflow owner escalation|boundary review|manual handoff|inspect|recover|owner_handoff|ready|needs_prep|approval gate evidence|workflow boundary evidence|recovery drill evidence|proof pointer|approval gate is current again|workflow boundary is current again|recovery drill state is current again|proof pointer is current again|approval gate fallback is open|boundary fallback is open|replay fallback is open|backup handoff is open|high|medium|low|silent_rehydrate|reopen_then_refresh|reload_before_run|refresh action|refresh first|refresh replay|refresh target state|refresh scope|after refresh|latest gate state|gate|boundary|proof|recovery|linked workflow boundary or workflow owner handoff|repo-owned recovery drill|gate fallback|handoff fallback|boundary fallback|replay fallback|open gate fallback|inspect boundary fallback|open replay fallback|inspect fallback escalation|hand off after fallback|open backup handoff|load the latest replay handoff before opening the backup handoff|open once the latest replay handoff is loaded|use manual follow-through if the backup handoff still does not restore the session path|escalate to manual handoff if the backup handoff still does not restore the session path/i);
