...
test("live desk and console surfaces prefer repo-owned runtime data with draft fallback support", () => {
  ...
  assert.match(consoleRuntime, /<ArtifactViewerPanel[\s\S]*initialArtifactPath=\{initialArtifactPath\}[\s\S]*initialArtifactIssue=\{initialArtifactIssue\}[\s\S]*initialCaseRef=\{runtimeCase\.ref\}[\s\S]*\/>/);
