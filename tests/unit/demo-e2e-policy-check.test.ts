...
test("demo-e2e policy check fails when gateway item-delete scenario attempts exceed configured retry max", () => {
  const result = runPolicyCheck(
    createPassingSummary({
      kpis: {
...
test("demo-e2e policy check fails when gateway task-progress scenario attempts exceed configured retry max", () => {
  ...
  assert.ok(Array.isArray(details?.violations));
  const violations = details.violations as string[];
  assert.ok(violations.some((item) => item.includes("kpi.gatewayTaskProgressScenarioAttempts")));
});

test("demo-e2e policy check fails when gateway request-replay scenario attempts exceed configured retry max", () => {
  const result = runPolicyCheck(
    createPassingSummary({
      kpis: {
  ...
  assert.ok(Array.isArray(details?.violations));
  const violations = details.violations as string[];
  assert.ok(violations.some((item) => item.includes("kpi.gatewayRequestReplayScenarioAttempts")));
});

test("demo-e2e policy check fails when gateway invalid-envelope scenario attempts exceed configured retry max", () => {
  const result = runPolicyCheck(
    createPassingSummary({
      kpis: {
  ...
  assert.ok(Array.isArray(details?.violations));
  const violations = details.violations as string[];
  assert.ok(violations.some((item) => item.includes("kpi.gatewayInvalidEnvelopeScenarioAttempts")));
});

test("demo-e2e policy check fails when gateway binding-mismatch scenario attempts exceed configured retry max", () => {
  const result = runPolicyCheck(
    createPassingSummary({
      kpis: {
  ...
  assert.ok(Array.isArray(details?.violations));
  const violations = details.violations as string[];
  assert.ok(violations.some((item) => item.includes("kpi.gatewayBindingMismatchScenarioAttempts")));
});

test("demo-e2e policy check fails when gateway draining-rejection scenario attempts exceed configured retry max", () => {
  const result = runPolicyCheck(
    createPassingSummary({
      kpis: {
  ...
  assert.ok(Array.isArray(details?.violations));
  const violations = details.violations as string[];
  assert.ok(violations.some((item) => item.includes("kpi.gatewayDrainingRejectionScenarioAttempts")));
});

test("demo-e2e policy check fails when multi-agent delegation scenario attempts exceed configured retry max", () => {
  const result = runPolicyCheck(
    createPassingSummary({
      kpis: {
  ...
  assert.ok(Array.isArray(details?.violations));
  const violations = details.violations as string[];
  assert.ok(violations.some((item) => item.includes("kpi.multiAgentDelegationScenarioAttempts")));
});

test("demo-e2e policy check fails when operator device-nodes lifecycle scenario attempts exceed configured retry max", () => {
  const result = runPolicyCheck(
    createPassingSummary({
      kpis: {
  ...
  assert.ok(Array.isArray(details?.violations));
  const violations = details.violations as string[];
  assert.ok(violations.some((item) => item.includes("kpi.operatorDeviceNodesLifecycleScenarioAttempts")));
});

test("demo-e2e policy check fails when approvals list scenario attempts exceed configured retry max", () => {
  const result = runPolicyCheck(
    createPassingSummary({
      kpis: {
  ...
  assert.ok(Array.isArray(details?.violations));
  const violations = details.violations as string[];
  assert.ok(violations.some((item) => item.includes("kpi.approvalsListScenarioAttempts")));
});

test("demo-e2e policy check fails when approvals invalid-intent scenario attempts exceed configured retry max", () => {
  const result = runPolicyCheck(
    createPassingSummary({
      kpis: {
...
test("demo-e2e policy check fails when gateway item-delete KPI is invalid", () => {
  const result = runPolicyCheck(
    createPassingSummary({
      kpis: {
        gatewayItemDeleteValidated: false,
      },
    }),
  );
  assert.equal(result.exitCode, 1);
