...
test("railway deploy helpers try API auth first, then legacy fallback, then active CLI session, then project-token fallback", () => {
  const gatewayScriptPath = resolve(process.cwd(), "scripts", "railway-deploy.ps1");
  const apiScriptPath = resolve(process.cwd(), "scripts", "railway-deploy-api.ps1");
  ...
  const apiScript = readFileSync(apiScriptPath, "utf8");
  const frontendScript = readFileSync(frontendScriptPath, "utf8");

  for (const source of [gatewayScript, apiScript, frontendScript]) {
    ...
    assert.match(source, /function Use-ActiveRailwayCliSession\(\[string\]\$Reason\)/);
    assert.match(source, /Using active Railway CLI session from 'railway login' after token auth failed\./);
    assert.match(source, /Using active Railway CLI session from 'railway login'\./);
    ...
    assert.match(source, /return \[pscustomobject\]@\{/);
    assert.match(source, /Output = \$probeText/);
    assert.match(source, /ExitCode = \$probeExitCode/);
    assert.doesNotMatch(source, /\$script:authProbe =/);
    assert.doesNotMatch(source, /\$script:authProbeExitCode =/);
...
test("readme documents api, legacy, active CLI session, and project-token deploy auth order", () => {
  ...
  assert.match(readme, /reuse an active local `railway login` session/i);
