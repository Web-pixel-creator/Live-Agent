...
Case Workspace now also chooses its default Case Wiki focus from repo-owned `highlights` plus backend-built `focusPack` when the operator has not explicitly selected a proof/question chip, so its drilldown, handoff preview, and active chip state open on the same compiled blocker/proof posture as the operator snapshot.
