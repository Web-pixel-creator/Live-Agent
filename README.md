...
Status strip readability is tuned for judge demos with higher neutral-text contrast, stronger card separation, pill-state color mapping for `Status`, `Session State`, `Mode`, `PTT`, and `Export`, plus stable `4x2` desktop layout (`3/2/1` responsive breakpoints).
...
`Story Timeline` empty states (preview + timeline list) include a visual `Timeline` cue card plus direct `Open Live Negotiator` and `Use Story Prompt Template` actions: the first jumps to live tab with `intent=story`, the second also seeds a ready-to-run story prompt.
`Device Nodes` tab shows a compact fleet summary (`total/online/degraded/offline/stale`), adds a `List Filter` (`all/online/degraded/offline/stale`) with live `Showing X of Y` counters, renders registered nodes as selectable status cards, and includes guided empty-state actions (`Use Demo Template` / `Show All Nodes`) for fast recovery.
