...
- `GET /v1/runtime/session-replay` (`x-operator-role`) -> compact session replay mirror for one selected session plus the current operator-safe workflow boundary; combines session status, recent runs/events, approvals, replay counters, resume-ready / blocked-by posture, latest verified proof pointer and stage, compact booking/follow-up/handoff metadata, a workflow boundary summary, a recovery path hint, and current workflow linkage without stitching `/v1/sessions`, `/v1/events`, `/v1/runs`, and `/v1/runtime/workflow-config` by hand.
