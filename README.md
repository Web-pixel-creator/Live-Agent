...
When `badge-details.json` carries `evidence.caseWikiEvidenceSignature`, the release gate now also checks its signed-vs-unsigned counts against `RUNTIME_EVIDENCE_SIGNING_ENABLED`, so dev runs can stay deterministically `unsigned` while keyed environments must prove `signedArtifacts=totalArtifacts`.
