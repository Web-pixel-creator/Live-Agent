...
Frontend includes an interactive `Story Timeline` panel/workspace (readiness `Timeline State` KPI + progress bar + `3x1` control row: scrubber/selector/current-position, followed by a full-width `Segment Preview` lane with asset refs + selectable segment cards) fed directly from Storyteller outputs, with explicit `timeline mode`, `asset mix`, and `segment progress` pills.
