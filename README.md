...
- `npm run runtime:surface:snapshot` -> writes `artifacts/runtime/runtime-surface-snapshot.json` with the repo-owned runtime surface inventory plus readiness snapshot; the live artifact also mirrors compact Case Wiki ingress provenance into `readiness.summary.workflow.caseWikiIngress`, while `-- --offline true` keeps a deterministic local/CI artifact without live service or device-node probing.
