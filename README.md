...
- Remote UI executor service: run `npm run dev:ui-executor`; endpoint `/execute` is used when `UI_NAVIGATOR_EXECUTOR_MODE=remote_http`, repo-owned runtime control surface is exposed via `GET /runtime/config` + `POST /runtime/control-plane-override` for force-simulation/sandbox drills, and long-horizon background browser worker execution is exposed via `POST /browser-jobs` plus `GET /browser-jobs/:jobId` and `POST /browser-jobs/:jobId/resume|cancel` for repo-owned resume/cancel actions. Stable `ref:` targets now also attempt one stale-selector recovery pass from ref id/name/label hints before surfacing `staleRefTargets`, and successful recoveries are reported through `grounding.healedRefTargets`.
