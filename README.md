...
Custom dropdown controls support keyboard navigation (`ArrowUp/ArrowDown/Home/End`, `Enter/Space`, `Escape`) and combobox/listbox ARIA semantics (`aria-controls`, `aria-expanded`, `aria-activedescendant`) for judge/operator accessibility.
...
Connection status metadata is rendered as a compact inline wrap-row (`Status`, `Assistant`, `Run ID`, `User ID`, `Session State`, `Mode`, `PTT`, `Export`) for faster scan and lower visual density.
...
Frontend includes an interactive `Story Timeline` panel/workspace (readiness `Timeline State` KPI + progress bar + segment scrubber + selector + preview + asset refs + selectable segment cards) fed directly from Storyteller outputs, with explicit `timeline mode`, `asset mix`, and `segment progress` pills.
`Story Timeline` empty states (preview + timeline list) include animated visual cue cards (`Storyboard` / `Timeline`), expected-output KPI chips (`Segments`, `Asset Mix`, `First Render`), readiness status strip (`timeline_idle / assets=none / progress=0%`), and collapsible guidance blocks (`Preview story arc and expected outputs`, `Preview example segment cards`) plus direct `Open Live Negotiator` and `Use Story Prompt Template` actions: the first is promoted as primary CTA and jumps to live tab with `intent=story`, the second seeds a ready-to-run story prompt.
When storyteller jobs are queued but segments have not arrived yet, the same empty states switch to a `pending` variant (`timeline_pending_video`, queue-focused hints, and rendering badges) to avoid a misleading “idle” appearance during first render.
`Storyteller` now also shows a top `Story Guidance` rail in idle state (title + hint + `Open Live Negotiator` / `Use Story Prompt Template`) so first-run actions are visible before scrolling to preview/list empty cards.
`Device Nodes` tab shows a compact fleet summary (`total/online/degraded/offline/stale`), adds `List Filter` (`all/online/degraded/offline/stale`) and `List Sort` (`last heartbeat/status priority/name A-Z/name Z-A`) controls with live `Showing X of Y` counters, renders registered nodes as selectable status cards, and includes guided empty-state actions (`Use Demo Template` / `Show All Nodes`) for fast recovery.
...
- Job summary includes badge evidence statuses for `operatorTurnTruncation`, `operatorTurnDelete`, `operatorDamageControl`, `governancePolicy`, `skillsRegistry`, `pluginMarketplace`, `deviceNodes`, `agentUsage`, and derived updates-lane signal `deviceNodeUpdatesStatus` from unified report `artifacts/release-evidence/report.json`.
