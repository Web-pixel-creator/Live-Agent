...
- Job summary includes strict badge evidence statuses from unified report `artifacts/release-evidence/report.json`: `operatorTurnTruncation`, `operatorTurnDelete`, `operatorDamageControl`, `governancePolicy`, `skillsRegistry`, `deviceNodes`, `agentUsage`, and derived updates-lane signal `deviceNodeUpdatesStatus`.
...
- Job summary includes badge evidence statuses for `operatorTurnTruncation`, `operatorTurnDelete`, `operatorDamageControl`, `governancePolicy`, `skillsRegistry`, `deviceNodes`, `agentUsage`, and derived updates-lane signal `deviceNodeUpdatesStatus` from unified report `artifacts/release-evidence/report.json`.
