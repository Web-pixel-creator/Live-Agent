...
Case workspace note: the primary case row now also adds a compact active-only `Task path` rail, so `Do now / Review next / Then continue` reads before the deeper drawer-routing context.
