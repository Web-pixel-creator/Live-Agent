...
When `badge-details.json` carries `evidence.caseWikiEvidenceSignature`, the release gate now also checks its signed-vs-unsigned counts against `RUNTIME_EVIDENCE_SIGNING_ENABLED`, so dev runs can stay deterministically `unsigned` while keyed environments must prove `signedArtifacts=totalArtifacts`. The same release evidence lane now also requires `evidence.caseWikiRoutingContext` to stay `pass` with `contextSource=case_wiki`, a non-empty focus/blocker/next-action tuple, and emitted routing metadata (`route`, `mode`, `requestedIntent`, `routedIntent`), so compiled memory routing adoption is proved instead of inferred.
...
- Job summary includes strict badge evidence statuses from unified report `artifacts/release-evidence/report.json`: `operatorTurnTruncation`, `operatorTurnDelete`, `operatorDamageControl`, `governancePolicy`, `skillsRegistry`, `pluginMarketplace`, `deviceNodes`, `agentUsage`, `runtimeGuardrailsSignalPaths`, `liveTransport`, `caseWikiEvidenceSignature`, `caseWikiRoutingContext`, `providerUsage`, and derived updates-lane signal `deviceNodeUpdatesStatus`. For `caseWikiRoutingContext`, the summary now also prints the compact routing tuple (`contextSource`, `focusId`, `blocker`, `nextAction`, `route`, `mode`, `requestedIntent`, `routedIntent`) so compiled-memory routing proof is readable directly from the workflow summary.
...
- Job summary includes badge evidence statuses for `operatorTurnTruncation`, `operatorTurnDelete`, `operatorDamageControl`, `governancePolicy`, `skillsRegistry`, `pluginMarketplace`, `deviceNodes`, `agentUsage`, `runtimeGuardrailsSignalPaths`, `liveTransport`, `caseWikiEvidenceSignature`, `caseWikiRoutingContext`, `providerUsage`, and derived updates-lane signal `deviceNodeUpdatesStatus` from unified report `artifacts/release-evidence/report.json`; for `caseWikiRoutingContext` it also prints the compact routing tuple (`contextSource`, `focusId`, `blocker`, `nextAction`, `route`, `mode`, `requestedIntent`, `routedIntent`), and the same summary also surfaces optional `railway-deploy-summary` / `repo-publish-summary` provenance when those files are present in the downloaded artifact bundle.
