...
That same `Focused Evidence` Signal context item now also shows a read-only `Source` subline, so operators can see where the signal came from one layer deeper in the console.
