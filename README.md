...
- Live direct bootstrap (api-backend/frontend): set `LIVE_DIRECT_MODE_ENABLED=true`, `LIVE_EPHEMERAL_TOKENS_ENABLED=true`, `LIVE_DIRECT_MODE_DEFAULT=direct_live|relay`, and `LIVE_DIRECT_TOKEN_TTL_SECONDS` to allow repo-owned `GET /v1/runtime/live/capabilities` plus `POST /v1/runtime/live/session-token` direct-live session bootstrap before falling back to relay.
