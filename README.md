...
- Browser worker snapshots also expose a compact replay bundle with target URL, verification posture, screenshot refs, result/checkpoint artifacts, and recovery posture (`retryCount`, stale refs observed/healed, resumed checkpoints, summary) so operators can inspect a UI run without opening raw logs first.
