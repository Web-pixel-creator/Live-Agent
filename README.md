...
- `GET /v1/runtime/case-wiki` (`x-operator-role`) -> repo-owned compiled case memory for one selected session; turns sessions, runs, approvals, workflow posture, operator notes, and recent runtime evidence into one structured `CaseWiki` snapshot (`overview`, `entities`, `timeline`, `compliance`, `auditLog`, `proofs`, `openQuestions`, `recommendedNextAction`, `handoffPack`, `detailPack`, `routingPack`, `actionPack`, `focusPack`, `previewPack`, `workspacePack`, `operatorPreviewPack`) without making the operator re-stitch raw logs. `workspacePack.defaultFocus` now carries the backend-owned Case Workspace default proof/question focus, `workspacePack.costSummary` adds a per-session token/live/ui/storage cost ledger for unit-economics proof and feeds the orchestrator cost guard when the snapshot is passed into an agent turn, `workspacePack.costValue` adds the compact card string for product/operator UI, `compliance` carries the active governance template, PII redaction level, retention posture, and evidence-signing expectation for the case, `operatorPreviewPack` carries compact `overview`, `evidence`, `questions`, `compliance`, `audit`, and `timeline` panes for `Operator Session Ops`, and `evidenceSignature` carries canonical SHA256 tamper evidence plus an Ed25519 signature when `RUNTIME_EVIDENCE_SIGNING_ENABLED=true` and `RUNTIME_EVIDENCE_SIGNING_PRIVATE_KEY_PEM` or `RUNTIME_EVIDENCE_SIGNING_PRIVATE_KEY_BASE64` is configured.
