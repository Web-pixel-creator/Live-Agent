...
When `Approvals`, `Runtime`, or `Audit` is active, the top operator toolbar now compresses automatically and hides broad `Board` / `Filters` chrome until the operator returns to `Overview`.
