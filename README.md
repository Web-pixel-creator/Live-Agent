...
The lower workflow tray now reads as `Workflow tools`, the control lane now keeps both `Operator approvals & queue` and `Operator diagnostics`, and the right rail stays focused on the latest result plus visible turns, so product-first language stays separate from governance and troubleshooting copy.
