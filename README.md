...
`GET /v1/operator/queue` now exposes the repo-owned operator queue compiled from `Case Wiki`, so the frontend `Active Queue` can prefer backend-prioritized remediation/approval/runtime actions and fall back to local `Case Wiki` inference only when the queue route is unavailable.
...
- `evidence.caseWikiRoutingContext` (`status`, `validated`, `observed`, `contextSource`, `focusId`, `blocker`, `nextAction`, `route`, `mode`, `requestedIntent`, `routedIntent`) for the orchestrator route that was executed with compiled `Case Wiki` memory as primary routing context
