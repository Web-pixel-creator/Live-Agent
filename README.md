...
Status strip readability is tuned for judge demos with higher neutral-text contrast, pill-state color mapping for `Status`, `Session State`, `Mode`, `PTT`, and `Export`, dedicated value chips for `Run ID`/`User ID`, lane-level `ok/neutral/fail` card accents, and extra mobile breathing room (larger tile padding/gaps); desktop keeps a `2x4` layout with adaptive `2-column`/`1-column` fallback on narrower screens.
...
`Story Timeline` empty states (preview + timeline list) include a visual `Timeline` cue card, expected-output KPI chips (`Segments`, `Asset Mix`, `First Render`), readiness status strip (`timeline_idle / assets=none / progress=0%`), and collapsible guidance blocks (`Preview story arc and expected outputs`, `Preview example segment cards`) plus direct `Open Live Negotiator` and `Use Story Prompt Template` actions: the first jumps to live tab with `intent=story`, the second also seeds a ready-to-run story prompt.
