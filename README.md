...
- Demo frontend Operator Console also includes a compact `Session Boundary` card, mirroring the latest loaded `GET /v1/runtime/session-replay` snapshot so operators can see selected-session replay posture, workflow boundary state, explicit `Boundary Owner`, `Approval Gate`, `Next Action`, `First Step`, `After refresh`, `Step Progress`, `Checklist`, `Latest Proof`, `Recovery`, and `Handoff` rows without reopening the deeper `Operator Session Ops` drawer; `First Step` stays canonical and now carries the repo-owned `active` plus `runnable` posture together with an `openable`/`executable` action mode, a `primed`/`not_primed` surface state, and a repo-owned `needsRefresh` freshness signal so primed surfaces can still report `fresh` vs `needs_refresh`, while stale primary steps also emit a repo-owned `refreshDisposition` (`silent_rehydrate`, `reopen_then_refresh`, or `reload_before_run`) together with a repo-owned `refreshEvidenceHint`, `refreshOutcomeLabel`, `refreshConfidence`, `refreshDetourHint`, `refreshEscalationHint`, `refreshEscalationTarget`, `refreshEscalationMode`, `refreshEscalationCTA`, `refreshEscalationReadiness`, `refreshEscalationPrepHint`, `refreshEscalationOpenGuard`, `refreshEscalationFallbackTarget`, `refreshEscalationFallbackCTA`, `refreshEscalationFallbackReadiness`, `refreshEscalationFallbackPrepHint`, `refreshEscalationFallbackOpenGuard`, `refreshEscalationFallbackOutcomeLabel`, `refreshEscalationFallbackConfidence`, `refreshEscalationFallbackDetourHint`, `refreshEscalationFallbackEscalationHint`, `refreshEscalationFallbackEscalationTarget`, `refreshEscalationFallbackEscalationMode`, `refreshEscalationFallbackEscalationCTA`, `refreshEscalationFallbackEscalationReadiness`, `refreshEscalationFallbackEscalationPrepHint`, `refreshAction`, `refreshTargetState`, and `refreshScope` so the CTA can switch to `Refresh first` and show which stale surface becomes current after refresh, what kind of refresh path it is, which operator evidence matters first, what successful refresh restores, how reliable that refresh is, whether a refresh detour still remains likely, whether the path should escalate after refresh instead of looping through another operator hop, exactly which escalation surface/posture comes next, what escalation mode that follow-up uses (`inspect`, `recover`, or `owner_handoff`), which refresh escalation CTA is the canonical next handoff after that stale refresh, whether that escalation path is already `ready` or still `needs_prep`, what prep is still missing before that escalation can open cleanly, what explicit condition makes that escalation safe to open, which fallback surface should take over if that escalation still does not open after prep, which fallback CTA is the canonical detour into that backup surface, whether that fallback path is already ready to open, what fallback prep is still missing before that detour can open cleanly, what explicit condition makes that fallback safe to open, what successful fallback opening restores, how reliable that backup detour is likely to be, whether that backup surface still suggests one more operator-owned detour after opening, when that same backup path should already flip into its own escalation path, exactly which operator surface/posture that fallback escalation should target next, what mode that fallback escalation uses (`inspect` or `owner_handoff` in the current repo-owned mapping), which fallback escalation CTA is the canonical handoff after that backup path escalates again, whether that fallback escalation follow-up is already `ready` or still `needs_prep`, what prep is still missing before that fallback escalation follow-up can open cleanly, and whether that refresh is reloading gate, boundary, proof, or recovery state instead of making operators guess between `Refresh Summary`, `Approvals`, `Workflow Control`, and `Session Ops`. The same primary-step contract now also exposes a structured repo-owned `refresh recovery followup path`, so `Session Boundary` renders a compact recovery ladder below `After refresh` and `Operator Session Ops` can export the same structure while legacy flat `refresh escalation ...` fields remain stable. Frontend read-models now prefer that structured ladder first and only fall back to legacy flat refresh-escalation hints when the ladder is absent. `Step Progress` shows the repo-owned progression index such as `1/3`, and `Checklist` reads as the remaining `queued`/`blocked` steps after that first move. The primary button now switches between `Refresh first`, `Open first step`, and `Run first step` from the repo-owned primary-step contract, follows the repo-owned next action target (`Approvals`, `Session Ops`, `Workflow Control`, or `Runtime Drill Runner`), and also switches the matching operator workspace posture before opening that deeper surface.
- The same replay contract now also includes a repo-owned `refresh escalation fallback escalation open guard`, so `needs_prep` fallback-escalation follow-ups state the exact safe-open condition before the operator tries that surface.
- The same replay contract now also includes a repo-owned `refresh escalation fallback escalation fallback target`, so the operator sees the backup surface if that fallback-escalation follow-up still cannot open cleanly.
- The same replay contract now also includes a repo-owned `refresh escalation fallback escalation fallback cta`, so that backup surface ships with one canonical operator handoff instead of a bare fallback target.
- The same replay contract now also includes a repo-owned `refresh escalation fallback escalation fallback readiness`, so the operator can see whether that backup handoff is already ready or still needs prep.
- The same replay contract now also includes a repo-owned `refresh escalation fallback escalation fallback prep hint`, so a `needs_prep` backup handoff explains which replay or handoff context is still missing.
...
6. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback outcome`, so the same backup handoff spells out the restored operator outcome once it opens cleanly.
7. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback confidence`, so the same backup handoff shows how reliably that backup follow-up should close the stale boundary once opened.
8. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback detour`, so the same backup handoff can tell the operator when one more manual follow-through is still needed after that backup path opens.
9. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation`, so the same backup handoff can tell the operator when that backup path should already flip into its own escalation path.
10. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation target`, so that same backup handoff escalation path can point at one exact next operator surface and posture instead of leaving the operator to infer it.
11. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation cta`, so that same backup handoff escalation path carries one canonical operator handoff label instead of leaving the follow-up action implicit.
12. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation readiness`, so that same backup handoff escalation path can say whether the follow-up is already `ready` or still `needs_prep`.
13. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation prep hint`, so that same backup handoff escalation path can explain what replay or handoff context is still missing before the follow-up is safe to open.
14. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation open guard`, so that same backup handoff escalation path can state the exact safe-open condition once the missing replay or handoff context is loaded.
15. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback target`, so that same nested backup-escalation follow-up can point at one spare operator surface if it still cannot open safely after preparation.
16. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback cta`, so that same nested spare surface carries one canonical operator handoff instead of leaving the follow-up action implicit.
17. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback readiness`, so that same nested spare surface can state whether that backup follow-up is already `ready` or still `needs_prep`.
18. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback prep hint`, so that same nested spare surface can explain what workflow, handoff, or recovery context is still missing before the follow-up is safe to open.
19. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback open guard`, so that same nested spare surface can state the exact safe-open condition once the missing workflow, handoff, or recovery context is loaded.
20. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback outcome`, so that same nested spare surface can spell out the restored operator outcome once that backup follow-through opens cleanly.
21. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback confidence`, so that same nested spare surface can show how reliably that backup follow-through should close the stale boundary once opened.
22. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback detour`, so that same nested spare surface can tell the operator when one more manual follow-through is still needed after that backup path opens.
23. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation`, so that same nested spare surface can tell the operator when that backup path should already flip into its own escalation path.
24. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation target`, so that same nested spare surface can point the operator to the exact next surface and posture for that escalation path.
25. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation cta`, so that same nested spare surface can hand the operator one canonical follow-through for that escalation path.
26. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation readiness`, so that same nested spare surface can state whether that escalation follow-through is already `ready` or still `needs_prep`.
27. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation prep hint`, so that same nested spare surface can explain what workflow, handoff, or recovery context is still missing before that escalation follow-through is safe to open.
28. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation open guard`, so that same nested spare surface can state the exact safe-open condition once the missing workflow, handoff, or recovery context is loaded.
29. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation outcome`, so that same nested spare surface can show the restored operator outcome once that escalation follow-through opens cleanly.
30. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation confidence`, so that same nested spare surface can show how reliably that escalation follow-through should close the stale boundary once opened.
31. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation detour`, so that same nested spare surface can tell the operator when one more owner-led follow-through is still needed after that escalation opens.
32. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation`, so that same nested spare surface can tell the operator when that escalation follow-through should already flip into one more explicit escalation path.
33. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation target`, so that same nested spare surface can point the operator to the exact next surface and posture for that escalation follow-through.
34. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation cta`, so that same nested spare surface can hand the operator one canonical follow-through for that escalation recovery path.
35. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation readiness`, so that same nested spare surface can state whether that escalation recovery follow-through is already `ready` or still `needs_prep`.
36. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation prep hint`, so that same nested spare surface can explain what workflow, handoff, or recovery context is still missing before that escalation recovery follow-through is safe to open.
37. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation open guard`, so that same nested spare surface can state the exact safe-open condition once the missing workflow, handoff, or recovery context is loaded.
38. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation outcome`, so that same nested spare surface can show the restored operator outcome once that escalation recovery follow-through opens cleanly.
39. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation confidence`, so that same nested spare surface can show how reliably that escalation recovery follow-through should close the stale boundary once opened.
40. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation detour`, so that same nested spare surface can tell the operator when one more ownership follow-through is still needed after that escalation recovery step opens.
41. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation`, so that same nested spare surface can tell the operator when that escalation recovery step should already flip into one more explicit escalation path.
42. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation target`, so that same nested spare surface can point the operator to the exact next surface and posture for that re-escalation path.
43. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation cta`, so that same nested spare surface can hand the operator one canonical follow-through for that re-escalation path.
44. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation readiness`, so that same nested spare surface can state whether that re-escalation follow-through is already `ready` or still `needs_prep`.
45. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation prep`, so that same nested spare surface can explain what still needs to be loaded before that re-escalation follow-through is safe to open.
46. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation open guard`, so that same nested spare surface can state the exact safe-open condition once that re-escalation context is finally loaded.
47. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation outcome`, so that same nested spare surface can show the restored operator posture once that re-escalation follow-through opens cleanly.
48. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation confidence`, so that same nested spare surface can show how reliably that re-escalation follow-through should close the stale boundary once opened.
49. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation detour`, so that same nested spare surface can tell the operator when one more ownership follow-through is still needed after that re-escalation step opens.
50. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation escalation`, so that same nested spare surface can tell the operator when that re-escalation step should already flip into one more explicit escalation path.
51. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation escalation target`, so that same nested spare surface can point the operator to the exact next surface and posture for that deeper re-escalation path.
52. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation escalation cta`, so that same nested spare surface can hand the operator one canonical follow-through for that deeper re-escalation path.
53. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation escalation readiness`, so that same nested spare surface can state whether that deeper re-escalation follow-through is already `ready` or still `needs_prep`.
54. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation escalation prep`, so that same nested spare surface can explain what still needs to be loaded before that deeper re-escalation follow-through is safe to open.
55. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation escalation open guard`, so that same nested spare surface can state the exact safe-open condition once that deeper re-escalation context is finally loaded.
56. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation escalation outcome`, so that same nested spare surface can show the restored operator posture once that deeper re-escalation follow-through opens cleanly.
57. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation escalation confidence`, so that same nested spare surface can show how reliably that deeper re-escalation follow-through should close the stale boundary once opened.
58. The runtime session replay mirror now also emits a repo-owned `refresh escalation fallback escalation fallback escalation fallback escalation escalation escalation escalation detour`, so that same nested spare surface can tell the operator which ownership check still needs follow-through after that deeper re-escalation opens.
59. The runtime session replay mirror now also emits a repo-owned structured `refresh recovery followup path`, so operator exports and future UI slices can consume one compact recovery ladder without hard-coding every flat `refresh escalation ...` field hop.
60. The runtime session replay mirror now also emits a repo-owned structured `refresh state`, so frontend/operator read-models can consume one nested refresh contract (`action`, `targetState`, summary hints, `followupPath`) while the older flat `refresh escalation ...` fields stay available as a compatibility layer.
61. That same structured `refresh state` now also emits a repo-owned nested `followupTree`, so backend/frontend cleanup can consume one compatibility tree instead of treating every flat `refresh escalation ...` hop as the primary model.
62. That same structured `refresh state` now also emits a repo-owned compatibility block, so consumers can prefer `refreshState.followupTree` while the flat `refreshEscalation...` projection stays transitional.
